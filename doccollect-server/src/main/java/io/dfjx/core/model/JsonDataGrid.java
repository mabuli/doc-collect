...
import io.dfjx.core.util.common.ReflectUtil;
...
public class JsonDataGrid {
    ...
    public String toJson(String dataFormatter){
    	...
    	if(!QCommon.isNullOrEmpty(dataFormatter)){
    		...
    		DateFormat format1 = new SimpleDateFormat(dataFormatter);
    		om.setDateFormat(format1);
    		json = JsonUtil.toJson(rows, om);
    	}
    	else{
    		json = JsonUtil.toJson(rows);
    	}
        return "{\"total\":" + total.toString() + ",\"rows\":" + json + "}";
    }
